...
;;; Index:
;;;	DOSLIB
;;;		DOSLIBLOADER - DOSLib loader according to AutoCAD version and system's processor architecture
;;;	OPENDCL
;;;		(to be done)
;;;	CFG/INI
...
;; returns the path and file name of loaded ARX, if succesful.
...
;; Returns: path of the INI file, if successful. Kill if not found.
  ...
  (if
    (findfile (setq *pathfilename (strcat gv:multidevpath "\\" gv:md_cfgfile)))
     ...
     (progn
       ...
       (setq gv:showsplashscreen (dos_getini "startup" "showsplashscreen" *pathfilename) ; show splashscreen
	     gv:loadmenu	 (dos_getini "interface" "loadmultidevmenu" *pathfilename) ; load MultiDEV menu
	     gv:menufile	 (dos_getini "interface" "multidevmenufile" *pathfilename) ; MultiDEV menu file name
	     gv:dwgscale	 (atof (dos_getini "drawing" "scale" *pathfilename)) ; base scale for drawing
	     gv:dwgunit		 (atoi (dos_getini "drawing" "unit" *pathfilename)) ; base unit for drawing
       ) ;_ setq
       ...
       (setq gv:md_settings (mapcar '(lambda (*key) ; record gv:md_settings
...
;; Returns: path of the cui file, if successful.
